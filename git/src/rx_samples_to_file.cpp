...
int UHD_SAFE_MAIN(int argc, char* argv[])
{
    ...
    desc.add_options()
        ("help", "help message")
        ("args", po::value<std::string>(&args)->default_value("name=MyB200_1"), "multi uhd device address args")
        ("file", po::value<std::string>(&file)->default_value("rx_data.bin"), "name of the file to write binary samples to")
        ("type", po::value<std::string>(&type)->default_value("float"), "sample type: double, float, or short")
        ("nsamps", po::value<size_t>(&total_num_samps)->default_value(4000), "total number of samples to receive")
        ("duration", po::value<double>(&total_time)->default_value(2), "total number of seconds to receive")
        ("time", po::value<double>(&total_time), "(DEPRECATED) will go away soon! Use --duration instead")
        ("spb", po::value<size_t>(&spb)->default_value(2500), "samples per buffer")
        ("rate", po::value<double>(&rate)->default_value(5.0e6), "rate of incoming samples")
        ("freq", po::value<double>(&freq)->default_value(2.6e9), "RF center frequency in Hz")
        ...
        ("bw", po::value<double>(&bw)->default_value(5e6), "analog frontend filter bandwidth in Hz")
